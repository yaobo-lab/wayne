...
func New(p string, options ...Option) (*RotateLogs, error) {
	...
	//匹配带数字后缀的日志文件，如：*.log.1
	globPattern = fmt.Sprintf("%s*", globPattern)
...
func (rl *RotateLogs) getWriter_nolock(bailOnRotateFail, useGenerationalNames bool) (io.Writer, error) {
	...
	if baseFn != rl.curBaseFn {
		generation = rl.getCurGeneration(baseFn)
...
func (rl *RotateLogs) rotate_nolock(filename string) error {
	...
	//排序
	toUnlink = rl.sortToUnlink(toUnlink)
...
//获取需要写入的当前日志文件索引号
func (rl *RotateLogs) getCurGeneration(baseFn string) int {
	//example: ./access_log.20220816.*
	globPattern := fmt.Sprintf("%s.*", baseFn)
	...
	for _, p := range matches {
		numStr := strings.TrimLeft(filepath.Ext(p), ".")
...
//匹配的日志文件排序（最小索引序号的放前面用于删除）
func (rl *RotateLogs) sortToUnlink(toUnlink []string) []string {
	type KVPair struct {
		Key string
		Val int64
	}
	...
	for _, path := range toUnlink {
		...
		tmpList = append(tmpList, KVPair{Key: path, Val: fi.ModTime().Unix()})
	}
	...
	sortResult := make([]string, len(tmpList))
	for i, pair := range tmpList {
		sortResult[i] = pair.Key
	}
	return sortResult
}
