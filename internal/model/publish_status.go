...
// 0:Deployment 1:Service 2:ConfigMap 3:Secret 4:PersistentVolumeClaim 5:CronJob 6:StatefulSet 7:DaemonSet 8:Ingress 9:HPA
const (
	//0
...
type PublishStatus struct {
	...
	AppId       int64       `orm:"index;column(app_id)" json:"app_id"`
	NamespaceId int64       `orm:"index;column(namespace_id);" json:"namespace_id"`
}
...
func (*publishStatusModel) Publish(m *PublishStatus) error {
	...
	if count == 0 {
		...
		if transactionError != nil {
			logger.Errorf("(%v) commit transaction error.%v", m, err)
		}
		return err
	}
	...
	for _, state := range publishStatus {
		...
		if err != nil {
			...
			if transactionError != nil {
				logger.Errorf("(%v) rollback transaction error.%v", m, err)
			}
			return err
		}
		transactionError := o.Commit()
