...
func addDefaultNamespace(user *User) (err error) {
	...
	if err == orm.ErrNoRows {
		...
		logger.Errorf("can not find default demo namespace by id " + beego.AppConfig.String("DemoNamespaceId"))
		return nil
	} else if err != nil {
		return
	}
	...
	if err == orm.ErrNoRows {
		// 如果没有初始化角色，则不再初始化
		logger.Errorf("can not find default demo group by id " + beego.AppConfig.String("DemoGroupId"))
		return nil
	} else if err != nil {
		return
	}
