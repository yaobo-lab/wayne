...
type Namespace struct {
	...
	Leader        string            `orm:"size(32)" json:"leader"`
...
func (*namespaceModel) GetNamespaceByAppId(appId int64) (*Namespace, error) {
	...
	if err != nil {
		logger.Warnf("get app by id (%d) error. %v", appId, err)
		return nil, err
	}

	ns, err := NamespaceModel.GetById(app.Namespace.Id)
	if err != nil {
		logger.Warnf("get namespace by id (%d) error. %v", app.Namespace.Id, err)
		return nil, err
	}
	var namespaceMetaData NamespaceMetaData
	err = json.Unmarshal(hack.Slice(ns.MetaData), &namespaceMetaData)
	if err != nil {
		logger.Errorf("Unmarshal namespace metadata (%s) error. %v", ns.MetaData, err)
		return nil, err
	}
