...
func (*deploymentModel) GetParseMetaDataById(id int64) (v *Deployment, err error) {
	...
	if err = Ormer().Read(v); err == nil {
		...
		if err != nil {
			logger.Errorf("parse deployment metaData error.", v.MetaData)
			return nil, err
		}
		return v, nil
	}
...
func (d *deploymentModel) Update(replicas int32, deploy *Deployment, cluster string) (err error) {
	...
	if err != nil {
		logger.Errorf("deployment metadata marshal error.%v", err)
		return
	}
	deploy.MetaData = string(newMetaData)
	err = d.UpdateById(deploy)
	if err != nil {
		logger.Errorf("deployment metadata update error.%v", err)
	}
	return
}
