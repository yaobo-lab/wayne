...
func daemonSetPreDeploy(kubeDaemonSet *appsV1.DaemonSet, daemonSet *model.DaemonSet,
	...
	if len(util.PodAnnotationControllerKindLabelKey) > 0 {
		kubeDaemonSet.Spec.Template.Annotations[util.PodAnnotationControllerKindLabelKey] = strings.ToLower(string(model.KubeApiTypeDaemonSet))
	}
}
