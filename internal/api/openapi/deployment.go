...
import (
	...
	resdeployment "wayne/internal/k8s/kind/deployment"
...
type DeploymentInfo struct {
	...
	DeploymentTemplete *model.DeploymentTemplate
...
type latestDeploymentTplResponse struct {
	...
	Body struct {
		dto.ResponseBase
		DeploymentTpl *model.DeploymentTemplate `json:"deployment_tpl"`
	}
}

// swagger:route GET /openapi/v1/gateway/action/get_deployment_status openapi DeploymentStatusParam
//
// 该接口用于返回特定部署的状态信息
//
// 重点关注 kubernetes 集群内状态而非描述信息，当然也可以只关注 healthz 字段。
...
func (c *OpenAPIController) GetDeploymentStatus() {
	...
	ns, err := model.NamespaceModel.GetByName(param.Namespace)
	if err != nil {
	...
	if err == nil {
		...
		for _, pod := range podInfo {
			...
			for _, status := range pod.ContainerStatus {
				p.ContainerStatus = append(p.ContainerStatus, response.ContainerStatus{Name: status.Name, RestartCount: status.RestartCount})
			}
...
// swagger:route GET /openapi/v1/gateway/action/restart_deployment openapi RestartDeploymentParam
//
// 用于用户调用以实现强制重启部署
//
...
func (c *OpenAPIController) RestartDeployment() {
	...
	deployResource, err := model.DeploymentModel.GetByName(param.Deployment)
	if err != nil {
	...
	if err := updateDeployment(deployObj, param.Cluster, deployResource.Id); err != nil {
...
// swagger:route GET /openapi/v1/gateway/action/upgrade_deployment openapi UpgradeDeploymentParam
//
// 用于 CI/CD 中的集成升级部署
//
// 该接口只能使用 app 级别的 apikey，这样做的目的主要是防止 apikey 的滥用。
...
func (c *OpenAPIController) UpgradeDeployment() {
	...
	if param.TemplateId != 0 && param.Publish {
		for _, cluster := range param.clusters {
			...
			utils.DeploymentPreDeploy(deployInfo.DeploymentObject, deployInfo.Deployment, deployInfo.Cluster, deployInfo.Namespace)
			err = publishDeployment(deployInfo)
			if err != nil {
	...
	for id, deployInfos := range deployInfoMap {
		...
		newtmplId, err := model.DeploymentTplModel.Add(deployInfo.DeploymentTemplete)
		if err != nil {
		...
		for k, deployInfo := range deployInfos {
			err := model.DeploymentModel.UpdateById(deployInfo.Deployment)
			if err != nil {
...
// swagger:route GET /openapi/v1/gateway/action/scale_deployment openapi ScaleDeploymentParam
//
...
func (c *OpenAPIController) ScaleDeployment() {
	...
	err = model.DeploymentModel.Update(replicas32, deployResource, param.Cluster)
	if err != nil {
		c.AddErrorAndResponse(fmt.Sprintf("Failed to update deployment in db! %s!", err.Error()), http.StatusInternalServerError)
		return
	}
	c.HandleResponse(nil)
}
...
func getOnlineDeploymenetInfo(deployment, namespace, cluster string, templateId int64) (deployInfo *DeploymentInfo, err error) {
	...
	deployResource, err := model.DeploymentModel.GetByName(deployment)
	if err != nil {
	...
	// 根据特定模板升级
	if templateId != 0 {
		deployInfo.DeploymentTemplete, err = model.DeploymentTplModel.GetById(int64(templateId))
		if err != nil {
		...
		// 获取并更新线上模板
		status, err := model.PublishStatusModel.GetByCluster(model.PublishTypeDeployment, deployResource.Id, cluster)
		if err != nil {
			return nil, fmt.Errorf("Failed to get publish status by cluster: %s", err.Error())
		}
		onlineTplId := status.TemplateId

		deployInfo.DeploymentTemplete, err = model.DeploymentTplModel.GetById(onlineTplId)
		if err != nil {
			return nil, fmt.Errorf("Failed to get deployment template by id: %s", err.Error())
		}
	}
	...
	// 拼凑 namespace 参数
	app, _ := model.AppModel.GetById(deployResource.AppId)
	...
	deployInfo.Cluster, err = model.ClusterModel.GetParsedMetaDataByName(cluster)
	if err != nil {
...
func publishDeployment(deployInfo *DeploymentInfo) error {
	...
	if err == nil {
		...
		if err != nil {
			...
			err := model.PublishStatusModel.Add(deployInfo.Deployment.Id, deployInfo.DeploymentTemplete.Id, deployInfo.Cluster.Name, model.PublishTypeDeployment)
			if err != nil {
				return err
			}
			return nil
...
func updateDeployment(deployObj *appsV1.Deployment, cluster string, resourceId int64) error {
	status, err := model.PublishStatusModel.GetByCluster(model.PublishTypeDeployment, resourceId, cluster)
	if err != nil {
	...
	if err != nil {
		...
		err := model.PublishStatusModel.Add(resourceId, status.TemplateId, cluster, model.PublishTypeDeployment)
		if err != nil {
			return err
		}
		return nil
...
// swagger:route GET /openapi/v1/gateway/action/get_deployment_detail openapi DeploymentDetailParam
//
...
func (c *OpenAPIController) GetDeploymentDetail() {
	...
	dep, err := model.DeploymentModel.GetUniqueDepByName(params.Namespace, params.App, params.Deployment)
	if err != nil {
...
// swagger:route GET /openapi/v1/gateway/action/get_latest_deployment_tpl openapi LatestDeploymentTplParam
//
...
func (c *OpenAPIController) GetLatestDeploymentTpl() {
	...
	dep, err := model.DeploymentTplModel.GetLatestDeptplByName(params.Namespace, params.App, params.Deployment)
	if err != nil {
		c.AddErrorAndResponse("Failed to get deployment by name!", http.StatusBadRequest)
		return
	}
