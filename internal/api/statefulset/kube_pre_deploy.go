...
func statefulsetPreDeploy(kubeStatefulSet *appV1.StatefulSet, statefulSet *model.Statefulset,

	cluster *model.Cluster, namespace *model.Namespace) {
	// step 1  add envs
	...
	if len(util.PodAnnotationControllerKindLabelKey) > 0 {
		kubeStatefulSet.Spec.Template.Annotations[util.PodAnnotationControllerKindLabelKey] = strings.ToLower(string(model.KubeApiTypeStatefulSet))
	}
}
