...
func DeploymentPreDeploy(kubeDeployment *appsV1.Deployment, deploy *model.Deployment,
	...
	if len(util.PodAnnotationControllerKindLabelKey) > 0 {
		kubeDeployment.Spec.Template.Annotations[util.PodAnnotationControllerKindLabelKey] = strings.ToLower(string(model.KubeApiTypeDeployment))
	}
}
