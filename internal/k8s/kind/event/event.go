...
func GetPodsWarningEvents(indexer *client.CacheFactory, pods []*apiv1.Pod) ([]k8sDto.Event, error) {
	events, err := indexer.EventLister().List(labels.Everything())
	if err != nil {
		return nil, err
	}

	result := make([]k8sDto.Event, 0)
	...
	for _, event := range events {
		result = append(result, k8sDto.Event{
...
func getPodsWarningEventsPage(kubeClient client.ResourceHandler, pods []*apiv1.Pod) ([]k8sDto.Event, error) {
...
func GetPodsEventByCronJobPage(kubeClient client.ResourceHandler, namespace, name string, q *dto.QueryParam) (*dto.Page, error) {
...
func pageResult(events []k8sDto.Event, q *dto.QueryParam) *dto.Page {
