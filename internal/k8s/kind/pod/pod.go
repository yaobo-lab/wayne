...
func filterPodByApiType(pods []*v1.Pod, apiType model.KubeApiType) []*v1.Pod {
...
func GetPodListPageByType(kubeClient client.ResourceHandler, namespace, resourceName string, resourceType api.ResourceName, q *dto.QueryParam) (*dto.Page, error) {
...
func getRelatedPodByTypeAndIntermediateType(kubeClient client.ResourceHandler, namespace, resourceName string,
	...
	for _, obj := range objs {
		commonObj, err := k8sDto.ToBaseObject(obj)
		if err != nil {
			return nil, err
		}
...
func pageResult(relatePod []*v1.Pod, q *dto.QueryParam) *dto.Page {
...
func GetPodsByDeployment(indexer *client.CacheFactory, namespace, name string) ([]*Pod, error) {
	...
	filteredPod := filterPodByApiType(pods, model.KubeApiTypeReplicaSet)
