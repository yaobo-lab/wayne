...
func ListPersistentVolume(cli *kubernetes.Clientset, listOptions metaV1.ListOptions) ([]v1.PersistentVolume, error) {
	pvList, err := cli.CoreV1().PersistentVolumes().List(context.Background(), listOptions)
	if err != nil {
		return nil, err
	}
...
func CreatePersistentVolume(cli *kubernetes.Clientset, pv *v1.PersistentVolume) (*v1.PersistentVolume, error) {
	pvCreated, err := cli.CoreV1().PersistentVolumes().Create(context.Background(), pv, metaV1.CreateOptions{})
	if err != nil {
		return nil, err
	}
...
func UpdatePersistentVolume(cli *kubernetes.Clientset, pv *v1.PersistentVolume) (*v1.PersistentVolume, error) {
	pvCreated, err := cli.CoreV1().PersistentVolumes().Update(context.Background(), pv, metaV1.UpdateOptions{})
	if err != nil {
		return nil, err
	}
	return pvCreated, nil
...
func DeletePersistentVolume(cli *kubernetes.Clientset, name string) error {
	return cli.CoreV1().PersistentVolumes().Delete(context.Background(), name, metaV1.DeleteOptions{})
}
