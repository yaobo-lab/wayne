...
func podUsedResourcesOnAvaliableNode(indexer *client.CacheFactory, avaliableNodeMap map[string]*v1.Node) (*k8sDto.ResourceList, error) {
	result := &k8sDto.ResourceList{}
	...
	for _, pod := range cachePods {
		...
		resourceList := k8sDto.ContainersRequestResourceList(pod.Spec.Containers)
...
func UpdateNode(cli *kubernetes.Clientset, node *v1.Node) (*v1.Node, error) {
	newNode, err := cli.CoreV1().Nodes().Update(context.Background(), node, metaV1.UpdateOptions{})
	if err != nil {
		return nil, err
	}
	return newNode, nil
...
func DeleteNode(cli *kubernetes.Clientset, name string) error {
	return cli.CoreV1().Nodes().Delete(context.Background(), name, metaV1.DeleteOptions{})
}
