...
func CreateOrUpdatePersistentVolumeClaim(cli *kubernetes.Clientset, pvc *kapi.PersistentVolumeClaim) (*kapi.PersistentVolumeClaim, error) {
	old, err := cli.CoreV1().PersistentVolumeClaims(pvc.Namespace).Get(context.Background(), pvc.Name, metaV1.GetOptions{})
	if err != nil {
		if kerrors.IsNotFound(err) {
			return cli.CoreV1().PersistentVolumeClaims(pvc.Namespace).Create(context.Background(), pvc, metaV1.CreateOptions{})
		}
		return nil, err
	}
	...
	return cli.CoreV1().PersistentVolumeClaims(pvc.Namespace).Update(context.Background(), old, metaV1.UpdateOptions{})
}
...
func DeletePersistentVolumeClaim(cli *kubernetes.Clientset, name, namespace string) error {
	return cli.CoreV1().PersistentVolumeClaims(namespace).Delete(context.Background(), name, metaV1.DeleteOptions{})
}
